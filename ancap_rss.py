...
def draw_feed(stdscr, entries, idx):
    ...
    stats_line = f"ARTÍCULOS: {total_count} | NO LEÍDOS: {unread_count} | FEEDS: {feeds_count}"
    if favorites:
        stats_line += f" | FAVORITOS: {len(favorites)}"
    ...
    if problematic_feeds:
        status_parts.append(f"⚠ {len(problematic_feeds)} feeds con problemas")
    if loading_done:
        status_parts.append("✓ ACTUALIZADO")
    else:
        status_parts.append("⟳ actualizando...")
    ...
    shortcuts_text = "j/k=nav SPACE=leer o=abrir f=fav l=save u=unread m=mark all q/ESC=salir PgUp/PgDn=scroll"
